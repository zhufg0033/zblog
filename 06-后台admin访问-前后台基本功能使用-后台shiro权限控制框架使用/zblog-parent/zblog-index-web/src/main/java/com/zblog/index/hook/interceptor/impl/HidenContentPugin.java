package com.zblog.index.hook.interceptor.impl;


import com.zblog.index.controller.ChannelController;
import com.zblog.index.hook.interceptor.InterceptorHookSupport;
...
public class HidenContentPugin extends InterceptorHookSupport {
    ...
    public void postHandle(HttpServletRequest request, HttpServletResponse response, HandlerMethod handler, ModelAndView modelAndView) throws Exception {
        ...
        if (null == editing && ret != null) {
            ...
            if (check(ret.getId(), ret.getAuthor().getId(), request)) {
    ...
    private boolean check(long id, long userId, HttpServletRequest request) {
